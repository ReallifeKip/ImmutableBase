...
use Tests\SingleValueObjects\SVO_3;
...
class DefaultTest extends TestCase
{
    ...
    public function testBasicWithCache()
    {
        ...
        shell_exec('php cacher ./tests');
        do {
            sleep(1);
        } while (!file_exists('cache.php'));
        StaticStatus::$properties = [];
        StaticStatus::$refs       = [];
        StaticStatus::$cachedMeta = require_once 'cache.php'; // NOSONAR
        $this->testBasic();
        StaticStatus::$cachedMeta = [];
        shell_exec('rm cache.php');
    }
    public function testLoadCacheSuccessfullyPopulatesStaticStatus()
    {
        $mockData = [
            'SomeClass' => ['methods' => [], 'properties' => []],
        ];

        $content = '<?php return ' . var_export($mockData, true) . ';';
        $file    = vfsStream::newFile('cache.php')
            ->at($this->root)
            ->setContent($content);
        $filePath = $file->url();

        ImmutableBase::loadCache($filePath);

        $this->assertEquals($mockData, StaticStatus::$cachedMeta);
    }
