...
abstract readonly class ImmutableBase
{
    ...
    final protected static function buildPropertyInheritanceChain(self $object): array
    {
        ...
        for ($ref = @StaticStatus::$refs[$static]; $ref && $ref?->name !== $self; $ref = $ref->getParentClass()) {
            $classname = $ref->name;
     ...
     *   3. Non-builtin enum → passthrough if object, otherwise analyzeEnum()
    ...
    private static function buildResolver(mixed $type, bool $isSub, bool $isSVO): callable
    {
        ...
        return match (true) {
            ...
            !$type['isBuiltin'] => match (true) {
                ...
                default => static fn(mixed $value) => \is_object($value) ? $value : self::analyzeEnum($typename, $value),
            },
    ...
    private static function valueDecide(array $type, mixed $value): mixed
    {
        if (!$type['isBuiltin']) {
            ...
            return match (true) {
                ...
                \is_string($value) && enum_exists($typename)            => self::analyzeEnum($typename, $value),
                \is_array($value) && is_a($typename, self::class, true) => $typename::fromArray($value),
                is_a($typename, SingleValueObject::class, true)         => $typename::from($value),
                default                                                 => throw new InvalidValueException($typename, $value)
            };
        }
        $typename = $type['typename']['string'];
     ...
     * @return void
     */
    final public static function loadCache(string $path): void
    {
